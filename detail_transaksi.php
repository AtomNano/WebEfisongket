...
$transactions = [];
if ($email) {
    $history_query = "SELECT t.created_at, oi.product_id, p.name AS product_name, oi.quantity, oi.price, t.status
                      FROM transactions t
            ...
            <div class="transaction-detail">
                ...
                <p><strong>Status:</strong> 
                    <span class="badge-status <?php 
                        if (strcasecmp($transaction['status'], 'Confirmed') === 0) echo 'badge-confirmed';
                        elseif (strcasecmp($transaction['status'], 'Pending') === 0) echo 'badge-pending';
                        elseif (strcasecmp($transaction['status'], 'Failed') === 0) echo 'badge-failed';
                    ?>">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php foreach ($order_items as $item): ?>
                            <tr>
                                <td><img src="./admin/uploads/<?php echo htmlspecialchars($item['product_image']); ?>" alt="Product Image" class="product-image"></td>
